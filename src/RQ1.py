...
class bellwether(object):
    ...
    def find_bellwether(self,data_source,path,fold):
        goals = ['recall','precision','pf']
        clusters = self.get_clusters(data_source)
        for cluster in clusters:
            if cluster.rsplit('/',1)[1] != '0':
                continue
            ...
            _df['wins'] = dom_score
            _df.to_csv(cluster + '/cdom_bellwether.csv')
...
if __name__ == "__main__":
    ...
    for _ in range(2):
        for train_index, test_index in kf.split(attr_df):
            ...
            bell.find_bellwether(data_store_path,path,i)
