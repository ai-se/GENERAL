...
class bellwether(object):
    ...
    def bellwether_without(self,selected_projects,all_projects):
        final_score = {}
        count = 0
    ...
    def bellwether_with(self,selected_projects,all_projects):
        final_score = {}
        count = 0
        for s_project in selected_projects:
            try:
    ...
    def run_bellwether_v2(self,_attr_df_train,cluster_ids,data_store_path):
        for cluster_id in cluster_ids:
            selected_projects = np.array(_attr_df_train.loc[cluster_tree[cluster_id].data_points].index)
            self.run(selected_projects,cluster_id,data_store_path)

    def run(self,selected_projects,cluster_id,data_store_path):
        print(cluster_id)
        final_score = self.bellwether_with(selected_projects,selected_projects)
    ...
    def find_bellwether_level2(self,data_source,path,fold):
        goals = ['recall','precision','pf']
        clusters = self.get_clusters(data_source)
        for cluster in clusters:
            if cluster.rsplit('/',1)[1] == 'results' or cluster.rsplit('/',1)[1] == 'cdom_level1' or cluster.rsplit('/',1)[1] == '0':
                continue 
            print(cluster)
    ...
    def calculate_level_1_performance(self,data_source,clusters,path,fold,cluster,cluster_tree):
        ...
        cluster_info = {}
        for cluster in clusters:
            if cluster.rsplit('/',1)[1] in ['results','cdom_level1','0']:
                continue
        ...
        for key in  level_1_bellwethers.keys():
            ...
            final_score = self.bellwether_with(sub_cluster_bellwethers,sub_cluster_bellwethers)

            if not Path(data_source + 'cdom_level1').is_dir():
                os.makedirs(Path(data_source + 'cdom_level1'))
    ...
    def find_bellwether_level0(self,data_source,path,fold,cluster,cluster_tree):
        ...
        final_score = self.bellwether_with(_cluster_bellwethers,_cluster_bellwethers)
        _results = {}
        for goal in goals:    
            for s_project in final_score.keys():
                if s_project not in _results.keys():
...
if __name__ == "__main__":
    ...
    for com_size in [50,100,200,400,attr_df_full.shape[0]]:
        attr_df = attr_df_full[0:com_size]
        ...
        for _ in range(2):
            for train_index, test_index in kf.split(attr_df):
                ...
                bell.find_bellwether_level2(data_store_path,path,i)
                _dir = data_store_path
                clusters = [(join(_dir, f)) for f in listdir(_dir) if Path(join(_dir, f)).is_dir()]
        ...
        print("Bellwether_finding time: ", mid - start)
